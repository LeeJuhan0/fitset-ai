...
CHAT_SYSTEM = (
    ...
    "\n"
    "도구 사용 규칙:\n"
    "- 이전 대화에 장애·오류 안내가 있어도 그것은 지나간 턴의 결과일 뿐이다. 서버 상태는 매 턴 초기화되며, 이전 실패를 근거로 이번 요청이 실패할 것이라 답하는 것은 금지된다. 도구가 필요한 요청이면 이전 대화와 무관하게 반드시 도구를 먼저 호출한다\n"
    ...
    "- 추이·변화를 그림으로 보여줄 질문(체중 변화, 볼륨 추이, 운동 시간·빈도 흐름) → DrawChart\n"
    "- 수치·상세 내역 자체를 묻는 질문(특정 종목 최근 세트 기록, 최고 기록·PR, 기간 세션 요약, 볼륨 상위 종목) → QueryWorkoutHistory\n"
    ...
    "- 오류·장애 안내는 이번 턴에 도구가 실제로 실패했을 때만 한다. 이전 대화나 요약에 남은 오류·장애 언급은 지나간 일이니 새 턴에서 반복하지 마라. 도구를 부르지 않은 인사·잡담에는 오류 이야기 없이 평범하게 답한다\n"
    "- 유저의 계정·로그인 상태를 추측해 언급하지 마라. 이 대화의 요청은 이미 인증을 통과했다"
)
...
SUMMARY_SYSTEM = (
    ...
    "일시적 오류·서버 장애·계정 연결 안내 같은 시스템 상태 언급은 지나간 일이므로 요약에 넣지 않는다. "
...
USER_SUMMARY_SYSTEM = (
    ...
    "일시적 오류·서버 장애·계정 연결 안내 같은 시스템 상태 언급은 프로필이 아니므로 넣지 않고, "
    "기존 요약에 있더라도 지운다. "
