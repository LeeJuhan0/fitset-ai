  ...
  "exercises": [
    {
      ...
      "orderIndex": 0,
      "sets": [ { "orderIndex": 0, "weight": 12.0, "reps": 15, "durationSeconds": 0 } ]
    }
  ]
}
```
...
2-A. **`weight` 단위는 기구 한 개 기준이다** — 덤벨·케틀벨은 한 손에 드는 한 개의 무게다. `weight: 14`면 14kg짜리 덤벨 두 개를 드는 것이지 합쳐서 14kg이 아니다. 바벨·머신·케이블은 봉 또는 스택에 걸린 총 무게 그대로다. 완료 기록을 저장할 때도 같은 기준으로 보낸다
2-B. **세트 구성은 종목 수행 방식(백엔드 `exerciseType`)에 따라 갈린다** (2026-08-05) — `WEIGHT_AND_REPS`는 `weight`+`reps`, `REPS_ONLY`(맨몸 스쿼트 등 32종)는 `reps`만(`weight` null), `DURATION`(플랭크·월싯 등 8종)은 `durationSeconds`만 — **종목별 기본값 × 난이도 계수**(초급 0.7·중급 1.0·고급 1.3, 5초 단위 반올림). 예: 핸드 플랭크 20/30/40초, 월 싯 30/45/60초, 스트레칭 15/20/25초. **세 필드는 모두 not null이고 해당 없는 자리는 `0`** — 클라가 null 분기 없이 0으로 판별한다 (2026-08-05 확정). 워밍업 첫 세트는 무게 종목이면 무게를, 시간 종목이면 시간을 절반으로 낮춘다. 백엔드 저장 API의 `InnerSetRequest`도 세 필드가 모두 선택이라 그대로 실어 보낼 수 있다
...
4. 루틴 저장은 클라가 `exercises[].exerciseId`(UUID)로 백엔드 `POST /api/v1/routines`(`RoutineCommitRequest` — exerciseId 필수)를 조립한다. 값은 백엔드 종목 마스터를 일 1회 배치로 캐시한 것(`exercise_catalog`) — 캐시 미스면 `null`이고 클라가 자체 종목 마스터로 매핑 폴백. AI 서버는 쓰기 없음
   ...
   > **세트 필드명 주의 (2026-08-05)** — 우리 payload는 스웨거 표기(`weight`·`reps`·`durationSeconds`)를 따르는데, 백엔드 루틴 저장 DTO의 실제 필드명은 `defaultWeight`·`targetReps`·`targetDurationSeconds`다. 스웨거가 루틴과 워크아웃의 동명 스키마(`InnerSetRequest`)를 하나로 합쳐 워크아웃 쪽을 노출하는 버그. 그대로 보내면 검증 애너테이션이 없어 **400 없이 세 값이 null로 저장**되므로, 백엔드가 이름을 통일하기 전까지 클라가 매핑해야 한다.
