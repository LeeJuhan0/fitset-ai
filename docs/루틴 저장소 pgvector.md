...
| 앱 연결 | SQLAlchemy `postgresql+psycopg` 엔진, pool_size 5, 세션 `default_transaction_read_only=on`, statement_timeout 2초. 엔티티 `routines.domain.Routine`(`pgvector.sqlalchemy.Vector`), 쿼리는 `select()` 조립 |
