"""채팅 도메인 순수 로직 테스트 — ULID·TTL·만료·정원·요약 판정."""
...
def _thread(thread_id: str, last_message_at: str | None, expires_at: int | None = None) -> dict:
    return {"thread_id": thread_id, "last_message_at": last_message_at, "expires_at": expires_at}
...
def test_iso_utc_uses_z_suffix():
    ...
    assert domain.iso_utc(NOW) == "2026-07-29T12:00:00Z"
...
def test_is_expired_only_when_ttl_passed():
    ...
    assert domain.is_expired(_thread("a", None, past), NOW) is True
    assert domain.is_expired(_thread("a", None, future), NOW) is False
    # TTL 속성이 없는 항목은 만료로 보지 않는다
    assert domain.is_expired(_thread("a", None, None), NOW) is False
